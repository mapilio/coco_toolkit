...
def draw_box_on_image(image_name, classes, colors, label_folder, raw_images_folder, save_images_folder):
    ...
    if image_name == ".DS_Store":
        return 0
    image_path = os.path.join(raw_images_folder, f"%s.{file_type}" % (image_name))  # 本次原始图片jpg路径

    save_file_path = os.path.join(save_images_folder, f"%s.{file_type}" % (image_name))  # 本次保存图片jpg路径
...
if __name__ == "__main__":  # 只有在文件作为脚本文件直接执行时才执行下面代码
    ...
    for image_name in image_names:  # 例遍图片名称
        box_num = draw_box_on_image(
            image_name, classes, colors, label_folder, raw_images_folder, save_images_folder
        )  # 对图片画框
        box_total += box_num
        image_total += 1
